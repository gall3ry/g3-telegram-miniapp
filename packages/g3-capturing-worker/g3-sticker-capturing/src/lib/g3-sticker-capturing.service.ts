...
export class G3StickerCapturingService {
  ...
  async getPng(payload: {
    stickerIds: number[];
  }): Promise<{ stickerId: number; cdnUrl: string }[]> {
    const db = this.db;

    Logger.debug(payload, `[getPng] Start`);
    const fn = z
      .function()
      .args(
        z.object({
        ...
        try {
          ...
          const result = await pqueue.addAll(
            stickers.map(({ id }) => async () => {
              ...
              const base64 = await page.evaluate(() => {
                return new Promise<string>((resolve, reject) => {
                  window.addEventListener('png', (e: unknown) => {
                    const gif = e as CustomEvent<string>;
                    resolve(gif.detail);
                  });
              ...
              const { url: cdnUrl } = await this._uploadPng({ base64 });
    ...
    Logger.debug(payload, `[getPng] End`);
    /// TODO: set stickers to telegram
    return result;
  }
  ...
  private _uploadPng(payload: { base64: string }) {
    return z
      .function()
      .args(
        z.object({
      ...
      .implement(async ({ base64 }) => {
        //data:image/gif;base64
        const file = base64.replace(/^data:image\/png;base64,/, '');
